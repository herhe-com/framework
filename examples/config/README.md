...
- `database.yaml`：MySQL、PostgreSQL、SQLite、Redis、MongoDB。
- `filesystem.yaml`：对象存储与磁盘映射。
- `auth.yaml`：JWT、Casbin、登录限制、权限树。
- `queue.yaml`：RabbitMQ 队列配置。
- `search.yaml`：Elasticsearch、Meilisearch。
...
4. `database.redis.<name>.db`、`filesystem.disks.<disk>`、`queue.rabbitmq.<name>` 这类 key 的层级要和这里保持一致。
